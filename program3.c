...
int stringToInt(const char *str) {
    if (str == NULL || *str == '\0') { // Sprawdzenie, czy łańcuch znaków jest pusty
        return -1;
    }
    ...
    if (endptr == str || *endptr != '\0') { // Sprawdzenie, czy nie nastąpił błąd podczas konwersji
        return -1;
    }
...
// Funkcja rozpoczęcia czytania
void start_reading() {
    pthread_mutex_lock(&mutex); // Zablokowanie muteksu

    if (waiting_writers > 0 || writting > 0) { // Jeśli są oczekujący pisarze lub ktoś pisze
...
// Funkcja rozpoczęcia pisania
void start_writting() {
    pthread_mutex_lock(&mutex); // Zablokowanie muteksu

    if (reading > 0 || writting > 0) { // Jeśli ktoś czyta lub pisze
...
void* reader(void *r) {
    ...
    while (1) {
        start_reading(); // Rozpoczęcie czytania
        random_wait_time(); // Symulacja czasu czytania
        stop_reading(); // Zakończenie czytania
...
void* writer(void *w) {
    srand(time(NULL)); // Inicjalizacja generatora liczb losowych
    while (1) {
        start_writting(); // Rozpoczęcie pisania
        random_wait_time(); // Symulacja czasu pisania
        stop_writting(); // Zakończenie pisania
...
int main(int argc, char *argv[]) {
    if (argc != 3) { // Sprawdzenie, czy podano poprawną liczbę argumentów
